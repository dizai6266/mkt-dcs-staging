...
# MAGIC %md
# MAGIC # Facebook Ad Spend Report
# MAGIC
# MAGIC 该 Notebook 从 Facebook Marketing API 获取广告分解的消耗数据。
# MAGIC
# MAGIC - 支持 publisher 和 age 两种分解维度
...
# 所有类别
ALL_CATEGORIES = [CATEGORY_PUBLISHER, CATEGORY_AGE]
...
try:
    ...
    dbutils.widgets.text("category", "", "Category (publisher/age, optional)")
    ds_param = dbutils.widgets.get("ds")
    category_param = dbutils.widgets.get("category")
except:
    ds_param = ""
    category_param = ""
...
print(f"📦 Category: {category_param if category_param else 'All categories'}")
...
def _get_insights(account_info, start_ds, ds, cfg):
    ...
    print(f'   📡 Getting insights for account: {account_id}, category: {category}')
    ...
    # 根据类别设置 breakdowns 和额外字段
    if category == CATEGORY_PUBLISHER:
...
def fetch_ad_spend_report_task(ds: str, category: str = None):
    """
    获取 Facebook Ad Spend 报告
    
    Args:
        ds: 执行日期 (YYYY-MM-DD)
        category: 指定的类别，如果为空则处理所有类别
    """
    print(f"📊 Fetching {_AD_NETWORK} ad spend report for {ds}")
    ...
    # 处理的类别列表
    categories_to_process = [category] if category else ALL_CATEGORIES
            ...
            for split_info in split_accounts:
                ...
                if account["account_id"] == split_account_id:
                    ...
                    while s_dt <= end_dt:
                        ...
                        tasks.append(((account, cat), s_dt.strftime('%Y-%m-%d'), c_dt.strftime('%Y-%m-%d'), cfg))
                        s_dt = c_dt + timedelta(days=1)
            
            if not is_split_account:
                tasks.append(((account, cat), start_ds, ds, cfg))
            ...
            for ac in account_list:
                if ac['account_id'] == account_info[2]:
                    ...
                    if report_info[1] != -1:
                        ...
                        helper.sql_error_bot(
                            title=f'Facebook spend report {cat}',
                            text=f'Facebook failed get_insights\n\n{account_info}'
                        )
        
        # 下载并保存报告
        for (category_item, report_id, account, start_ds_item, ds_item) in report_accounts:
    ...
    print(f"\n✅ Saved {_AD_NETWORK} ad spend report for {start_ds} to {ds}")
...
print(f"🚀 Starting Job for {_AD_NETWORK} Ad Spend")

try:
    # 如果指定了 category，只处理该 category；否则处理所有
    fetch_ad_spend_report_task(ds_param, category_param if category_param else None)
    print("\n✅ Job Finished Successfully")

except Exception as e:
    print(f"\n❌ Job Failed: {e}")
    # on_failure_callback: 失败时发送飞书通知
    helper.failure_callback(str(e), f"{_AD_NETWORK}_ad_spend_report")
    raise e
