...
# MAGIC %md
# MAGIC # Google Mediation AdMob Report
# MAGIC
# MAGIC 该 Notebook 从 Google AdMob API 获取 Mediation 报告数据。
# MAGIC
# MAGIC - 支持多账号配置
# MAGIC - 获取 Mediation 维度数据（AD_SOURCE, AD_SOURCE_INSTANCE, MEDIATION_GROUP）
...
def _fetch_mediation_report(ds: str, account_name: str, adunits: list, admob, account: str):
    """
    获取 Mediation 报告
    ...
    for ad_uni_id in adunits: 
        report_spec = {
            ...
            'dimensions': [
                'DATE', 'AD_SOURCE', 'AD_SOURCE_INSTANCE', 'APP', 
                'AD_UNIT', 'MEDIATION_GROUP', 'COUNTRY', 'FORMAT', 'PLATFORM'
            ],
            'metrics': [
                'AD_REQUESTS', 'MATCHED_REQUESTS', 'CLICKS', 'ESTIMATED_EARNINGS', 
                'IMPRESSIONS', 'IMPRESSION_CTR', 'MATCH_RATE', 'OBSERVED_ECPM'
            ],
        ...
        result = admob.accounts().mediationReport().generate(
    ...
    # 使用 'google' 作为 ad_network 以保持与原始逻辑一致
            ...
            for i in range(0, len(data)):
                ...
                if item.get('row'):
                    ...
                    for key in dimensions.keys():
                        ...
                        elif key in ('APP', 'AD_UNIT', 'AD_SOURCE', 'AD_SOURCE_INSTANCE', 'MEDIATION_GROUP'):
...
def fetch_mediation_report_task(ds: str):
    """
    获取 Google Mediation AdMob 报告
    ...
    for account_name in ACCOUNT_NAMES:
        ...
        try:
            ...
            # Step 2: 获取报告
            print(f"      📊 Fetching mediation reports...")
            file_path = _fetch_mediation_report(ds, account_name, adunits, admob, account)
    ...
    print(f"\n✅ Completed {_AD_NETWORK} report for {ds}")
...
try:
    fetch_mediation_report_task(ds_param)
    print("\n✅ Job Finished Successfully")

except Exception as e:
    print(f"\n❌ Job Failed: {e}")
    # on_failure_callback: 失败时发送飞书通知
    helper.failure_callback(str(e), f"{_AD_NETWORK}_admob_report")
    raise e
...
helper.validate_and_preview_data(_AD_TYPE, 'google')
