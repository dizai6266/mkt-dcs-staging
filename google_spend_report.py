...
# MAGIC %md
# MAGIC # Google Ads Spend Report
# MAGIC
# MAGIC 该 Notebook 从 Google Ads API 获取广告消耗数据。
# MAGIC
# MAGIC - 支持多客户层级账号
# MAGIC - 获取 location_view 数据
# MAGIC - 包含 Campaign、Location、Metrics 等维度
...
def _issue_search_request(client, customer_id, query):
    ...
    while True:
        try:
            ...
            result_strings = []
            result_strings.append('Day,Location,Is negative,Campaign ID,Campaign,Campaign state,Impressions,Clicks,Cost,Avg. Cost,Conversions,Currency,Account,Time zone,Client name,Customer ID')
            
            for batch in stream:
                for row in batch.results:
                    result_string = f"{row.segments.date},{row.campaign_criterion.criterion_id},{row.campaign_criterion.negative},{row.campaign.id},{row.campaign.name},{row.campaign.status},{row.metrics.impressions},{row.metrics.clicks},{row.metrics.cost_micros},{row.metrics.average_cost},{row.metrics.conversions},{row.customer.currency_code},{row.customer.descriptive_name},{row.customer.time_zone},{row.customer.descriptive_name},{row.customer.id}"
...
def _fetch_spend_report(ds: str, customer_id: tuple):
    ...
    campaign_query = """
        SELECT 
            segments.date, campaign_criterion.criterion_id, campaign_criterion.negative, 
            campaign.id, campaign.name, campaign.status, 
            metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.average_cost, 
            metrics.conversions, metrics.ctr, 
            customer.currency_code, customer.descriptive_name, customer.time_zone, 
            campaign_criterion.bid_modifier, customer.descriptive_name, customer.id, 
            segments.month, segments.quarter, segments.week, segments.year, 
            metrics.all_conversions_from_interactions_rate, metrics.all_conversions, 
            metrics.all_conversions_value, metrics.average_cpc, metrics.average_cpe, 
            metrics.average_cpm, metrics.average_cpv, metrics.conversions_from_interactions_rate, 
            metrics.conversions_value, metrics.cost_per_all_conversions, metrics.cost_per_conversion, 
            metrics.cross_device_conversions, metrics.engagement_rate, metrics.engagements, 
            ...
            metrics.value_per_conversion, metrics.video_view_rate, metrics.video_views, 
